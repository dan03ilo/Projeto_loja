...
if(!empty($data->nome) && !empty($data->cpf) && !empty($data->id_endereco) && !empty($data->id_contato) && !empty($data->id_usuario) && !empty($data->id)){
    ...
    if($cliente->alterarCliente()){
        ...
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possível atualizar o cliente."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para atualizar o cliente"));
}
?>