...
if(!empty($data->id_pedido) && !empty($data->valor) && !empty($data->formapagamento) && !empty($data->descricao)  && !empty($data->numeroparcelas) && !empty($data->valorparcela) && !empty($data->id)){
    ...
    if($pagamento->alterarPagamento()){
        ...
        header("HTTP/1.0 400");
        echo json_encode(array("mensagem"=>"Não foi possível atualizar o endereço."));
    }
}
else{
    header("HTTP/1.0 400");
    echo json_encode(array("mensagem"=>"Você precisa passar todos os dados para atualizar o endereço"));
}
?>